    ...
    public function buy(){
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            if ($resp == TRUE) {
                ...
                $this->session->set_flashdata('alertMsg', 'Logged In');

                redirect('home/product');
            } else {
                $this->session->set_flashdata('alertMsg', 'Invalid Email or Password');
                redirect('home/buy');
            }
        }
...
public function signup()
{
    ...
    if ($this->form_validation->run() == TRUE) {
        ...
        if ($resp == TRUE) {
            $this->session->set_flashdata('alertMsg', 'Signed In successfully.');
            redirect('home/buy');
        } else {
            $this->session->set_flashdata('alertMsg', 'Something went wrong');
...
public function bindCart()
{
    ...
    <thead>
    <tr>
    <th style="font-weight:300;">Quantity</th>
    <th style="font-weight:300;">Item Name</th>
    <th style="font-weight:300;">Item Price</th>
    </tr>
    </thead>';
    foreach ($cart_check as $key => $value) {
        $total = $total + $value['subtotal'];
        ...
        $table .= '<tbody id="cart_content">
        <tr>
        <th style="font-weight:300;">'.$value['qty']. '</th>
        <th style="font-weight:300;">'.$value['name']. '</th>
        <th style="font-weight:300;"><i class="fa fa-inr text_red"></i>'.$value['price'].'</th>
        </tr>
        </tbody>';
    }
    $table .= '<tfoot>
    <tr>
    <th colspan="3" align="right" style="font-weight:300;">
    Total: &nbsp;<i class="fa fa-inr text_red"></i>'.max($subtotal).'
    <input type="hidden" name="sub_total" id="sub_total" value="'.max($subtotal).'">
    </th>
    </tr>
    ...
    $table .= '<a href="'.base_url().'home/cart" class="btn btn-warning" style="border-radius:0px;margin-bottom:2vh;margin-left:3vw;margin-left: 10px;margin-right:10px;">View Cart</a>';
    $table .= '<a href='.base_url().'home/checkout class="btn btn-warning checkout" style="border-radius:0px;margin-bottom:2vh;margin-right:1vw;float:right;background-color: orange;">Proceed to Checkout</a>';
    echo json_encode($table);
}
...
public function order()
{
    ...
    public function view_order(){
        $resp['userData'] = $this->my_model->view_order();
        // print_r($resp); die();
        $this->load->view('home/view_order',$resp);
    }
    ...
    public function account(){
        ...
        $data['users'] = $this->my_model->account($id);
        // print_r($data['users']);
    ...
    {
        $data['userData'] = $this->my_model->edit($id);
        $this->form_validation->set_error_delimiters('<p class="text-danger">', '</p>');
        $this->form_validation->set_rules('firstname', 'First Name', 'required');
        $this->form_validation->set_rules('lastname', 'Last Name', 'required');
        $this->form_validation->set_rules('email', 'Email', 'required|valid_email');
        $this->form_validation->set_rules('password','password','required');
        $this->form_validation->set_rules('mobileNo', 'Mobile No.', 'required|regex_match[/^[0-9]{10}$/]');
        $this->form_validation->set_rules('uploadFile', 'IMAGE', 'callback_checkImageType[uploadFile]');
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            if (empty($_FILES['uploadFile']['name'])) {
                $this->form_validation->set_rules('uploadFile', 'IMAGE', 'required');
            }
            if ($this->form_validation->run() == TRUE) {
                $this->load->library('upload');
                if ($_FILES["uploadFile"]["name"] != '') {
                    $resp = doUploadFile('uploadFile', './assets/uploads/');
                    if ($resp['status']) {
                        // if (!empty($data['ad']->signUpPage)) {
                        //     deleteFile('assets/uploads/advertisment/', $data['ad']->signUpPage);
                        // }
                        $result['uploadFile'] = $resp['uploadData']['file_name'];
                        $resp = $this->my_model->upload($result);
                    } else {
                        //$this->session->set_flashdata('error_msg', $resp['error']);
                        redirect('home/account');
                    }
                }
            }
            $resp = $this->my_model->update($result);
            // print_r($resp); die("HELLO");
            if ($resp == TRUE) {
                $this->session->set_flashdata('alertMsg', 'Data updated successfully.');
                redirect('home/account');
            } else {
                $this->session->set_flashdata('alertMsg', 'Data not updated successfully.');
                redirect('home/account');
            }
        }
    ...
    public function checkImageType($file = '', $fileName = 'uploadFile')
    {
        if (isset($_FILES[$fileName]["name"])) {
            if (!empty($_FILES[$fileName]["name"])) {
                ...
                if ($type[1] == 'png' || $type[1] == 'jpg' || $type[1] == 'jpeg') {
                    ...
                    $this->form_validation->set_message('checkImageType', 'Please upload an image with Valid File Type.');
                    return FALSE;
                }
            } else {
                return TRUE;
